...
> [!NOTE]
> Those functions are intended for advanced users that want full control over the frame processing. By default `EndDrawing()` does this job: draws everything + `SwapScreenBuffer()` + manages frame timing + polls input events. To avoid that behaviour and control frame processes manually, enable in `config.h`: `SUPPORT_CUSTOM_FRAME_CONTROL`.
...
> [!NOTE]
> It can be useful when using basic shapes and one single font, defining a font char white rectangle would allow drawing everything in a single draw call
