  ...
  for (int i = 0; i < simulatorState->connectionCount; ++i) {
    if (simulatorState->connections[i].isActive) {
      ...
      if (fromElement && toElement) {
        ...
        STUB("For elements with multiple inputs/outputs, adjust start/end "
...
static void DrawScenarioDetailsScreen(const SimulatorState *simulatorState) {
  ...
  Rectangle closeBtn = {currentScreenWidth - closeBtnW - UI_PADDING, UI_PADDING,
                        closeBtnW, closeBtnH};
  if (DrawUIButton(closeBtn, "Return", LIGHTGRAY, COLOR_TEXT_PRIMARY)) {
    currentClientScreen = CLIENT_SCREEN_SIMULATION;
    TraceLog(LOG_INFO,
             "CLIENT: Closing Scenario Details view, returning to Simulation");
  }

  const char *title = TextFormat("Details for Scenario: %s",
                                 simulatorState->currentScenario.name);
  Vector2 titleSize = MeasureTextEx(clientFont, title, 30, 2);
  DrawTextEx(clientFont, title,
             (Vector2){(currentScreenWidth - titleSize.x) / 2, UI_PADDING * 2},
  ...
  DrawTextEx(
      ...
      (Vector2){(currentScreenWidth - instructionsSize.x) / 2,
                currentScreenHeight - UI_PADDING * 2 - instructionsSize.y},
      20, 1, COLOR_TEXT_SECONDARY);

  float sectionPadding = 20;
  float sectionWidth = (currentScreenWidth - 4 * sectionPadding) / 3;
  float sectionHeight =
      currentScreenHeight - UI_PADDING * 8 - titleSize.y - instructionsSize.y;
  ...
  DrawTextEx(clientFont, "System States (Operational Flow)",
             (Vector2){fsmRect.x + 10, fsmRect.y + 10}, 18, 1,
             COLOR_TEXT_PRIMARY);
  TODO("Draw placeholder FSM diagram: circles, arrows, state labels");
  DrawCircle((int)(fsmRect.x + fsmRect.width / 2 - 50),
             (int)(fsmRect.y + fsmRect.height / 2), 30, LIGHTGRAY);
  DrawCircle((int)(fsmRect.x + fsmRect.width / 2 + 50),
             (int)(fsmRect.y + fsmRect.height / 2 - 60), 30, LIGHTGRAY);
  DrawLineEx((Vector2){fsmRect.x + fsmRect.width / 2 - 20,
                       fsmRect.y + fsmRect.height / 2},
             (Vector2){fsmRect.x + fsmRect.width / 2 + 20,
                       fsmRect.y + fsmRect.height / 2 - 50},
             2, DARKGRAY);

  Rectangle truthTableRect = {sectionPadding * 2 + sectionWidth, sectionY,
                              sectionWidth, sectionHeight};
  DrawRectangleLinesEx(truthTableRect, 2, DARKGRAY);
  DrawTextEx(clientFont, "Signal Logic (Behavior Matrix)",
             (Vector2){truthTableRect.x + 10, truthTableRect.y + 10}, 18, 1,
             COLOR_TEXT_PRIMARY);
  TODO("Draw placeholder Truth Table: grid, headers (Input A, Input B, "
       "Output), values (0/1)");
  DrawLine((int)truthTableRect.x + 10, (int)truthTableRect.y + 80,
           (int)truthTableRect.x + (int)truthTableRect.width - 10,
           (int)truthTableRect.y + 80, DARKGRAY);
  DrawLine((int)truthTableRect.x + (int)truthTableRect.width / 2,
           (int)truthTableRect.y + 40,
           (int)truthTableRect.x + (int)truthTableRect.width / 2,
           (int)truthTableRect.y + (int)truthTableRect.height - 10, DARKGRAY);
  DrawTextEx(clientFont, "In1 | Out",
             (Vector2){truthTableRect.x + 20, truthTableRect.y + 50}, 16, 1,
             COLOR_TEXT_SECONDARY);
  DrawTextEx(clientFont, " 0  |  1 ",
             (Vector2){truthTableRect.x + 20, truthTableRect.y + 90}, 16, 1,
             COLOR_TEXT_SECONDARY);

  Rectangle circuitRect = {sectionPadding * 3 + sectionWidth * 2, sectionY,
                           sectionWidth, sectionHeight};
  DrawRectangleLinesEx(circuitRect, 2, DARKGRAY);
  DrawTextEx(clientFont, "Element Configuration (Layout)",
             (Vector2){circuitRect.x + 10, circuitRect.y + 10}, 18, 1,
             COLOR_TEXT_PRIMARY);
  TODO("Draw placeholder Circuit Diagram: component-like boxes, connection "
       "lines");
  DrawRectangle((int)(circuitRect.x + circuitRect.width / 2 - 60),
                (int)(circuitRect.y + circuitRect.height / 2 - 20), 40, 40,
                LIGHTGRAY);
  DrawRectangle((int)(circuitRect.x + circuitRect.width / 2 + 20),
                (int)(circuitRect.y + circuitRect.height / 2 - 20), 40, 40,
                LIGHTGRAY);
  DrawLineEx((Vector2){circuitRect.x + circuitRect.width / 2 - 20,
                       circuitRect.y + circuitRect.height / 2},
             (Vector2){circuitRect.x + circuitRect.width / 2 + 20,
                       circuitRect.y + circuitRect.height / 2},
             2, DARKGRAY);
}
...
static void DrawGameplayScreen(const SimulatorState *simulatorState) {
  ...
  Rectangle deckArea = {0, currentScreenHeight - UI_DECK_AREA_HEIGHT,
                        currentScreenWidth, UI_DECK_AREA_HEIGHT};
  Rectangle playArea = {0, UI_HEADER_HEIGHT, currentScreenWidth,
                        currentScreenHeight - UI_HEADER_HEIGHT -
                            UI_DECK_AREA_HEIGHT};
  ...
  DrawTextEx(clientFont,
             TextFormat("Scenario: %s", simulatorState->currentScenario.name),
             (Vector2){headerArea.x + UI_PADDING, headerTextY},
             scenarioNameSize, 2, COLOR_TEXT_PRIMARY);

  if (simulatorState->currentScenario.isCompleted) {
    DrawTextEx(clientFont, "COMPLETED!",
               (Vector2){headerArea.x + 400, headerTextY}, scenarioNameSize, 2,
               GREEN);
  }

  const char *statusText = TextFormat(
      "Deck: %d | Discard: %d | Turn: %s | Actions: %d/%d",
      simulatorState->deckCardCount - simulatorState->currentDeckIndex,
      simulatorState->discardCardCount, turnInProgress ? "Active" : "Ended",
      actionsThisTurn, maxActionsPerTurn);
  Vector2 statusTextDimensions =
      MeasureTextEx(clientFont, statusText, statusTextSizeVal, 1);
  DrawTextEx(clientFont, statusText,
             (Vector2){currentScreenWidth - statusTextDimensions.x - UI_PADDING,
                       headerArea.y +
                           (UI_HEADER_HEIGHT - statusTextDimensions.y) / 2.0f},
             statusTextSizeVal, 1, COLOR_TEXT_SECONDARY);
  float conditionsStartX = headerArea.x + UI_PADDING;
  float conditionsStartY =
      headerTextY + scenarioNameSize +
      (simulatorState->currentScenario.isCompleted ? scenarioNameSize + 4 : 5);
  for (int i = 0; i < simulatorState->currentScenario.conditionCount; ++i) {
    if (conditionsStartY + (i * (conditionSize + 2)) + conditionSize <
        headerArea.y + UI_HEADER_HEIGHT - UI_PADDING) {
      ScenarioCondition condition =
          simulatorState->currentScenario.conditions[i];
      Color conditionColor = condition.isMet ? GREEN : COLOR_TEXT_SECONDARY;
      const char *statusIcon = condition.isMet ? "[X]" : "[ ]";

      DrawTextEx(clientFont,
                 TextFormat("%s %s", statusIcon, condition.description),
                 (Vector2){conditionsStartX,
                           conditionsStartY + (i * (conditionSize + 2))},
                 conditionSize, 1, conditionColor);
    }
  }
  ...
  if (detailsButtonX + detailsButtonWidth >
      currentScreenWidth -
          MeasureTextEx(clientFont, statusText, statusTextSizeVal, 1).x -
          UI_PADDING - 10) {
    detailsButtonX =
        headerArea.x + UI_PADDING +
        MeasureTextEx(
            ...
            .x +
        20;
    if (detailsButtonX + detailsButtonWidth > currentScreenWidth / 1.5f) {
      detailsButtonX = currentScreenWidth / 2.0f;
    }
  }

  Rectangle detailsButtonRect = {detailsButtonX, headerTextY,
                                 detailsButtonWidth, detailsButtonHeight};

  if (CheckCollisionPointRec(GetMousePosition(), detailsButtonRect) &&
      IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
    currentClientScreen = CLIENT_SCREEN_SCENARIO_DETAILS;
    TraceLog(LOG_INFO, "CLIENT: Opening Scenario Details view.");
  }

  DrawRectangleRec(detailsButtonRect,
                   CheckCollisionPointRec(GetMousePosition(), detailsButtonRect)
                       ? COLOR_ACCENT_SECONDARY
                       : LIGHTGRAY);
  DrawRectangleLinesEx(detailsButtonRect, 1, DARKGRAY);
  const char *detailsButtonText = "[View Details]";
  Vector2 detailsButtonTextSize =
      MeasureTextEx(clientFont, detailsButtonText, 18, 1);
  DrawTextEx(
      clientFont, detailsButtonText,
      (Vector2){detailsButtonRect.x +
                    (detailsButtonRect.width - detailsButtonTextSize.x) / 2,
                detailsButtonRect.y +
                    (detailsButtonRect.height - detailsButtonTextSize.y) / 2},
  ...
  DrawTextEx(clientFont,
             TextFormat("Hand (%d/%d):", simulatorState->handCardCount,
                        MAX_CARDS_IN_HAND),
             (Vector2){deckArea.x + UI_PADDING, handLabelY}, 20, 1,
             COLOR_TEXT_PRIMARY);

  const char *handLabelText = TextFormat(
      "Hand (%d/%d):", simulatorState->handCardCount, MAX_CARDS_IN_HAND);
  Vector2 handLabelSize = MeasureTextEx(clientFont, handLabelText, 20, 1);
  float handLabelTextWidth = handLabelSize.x;

  DrawTextEx(clientFont, handLabelText,
             (Vector2){deckArea.x + UI_PADDING, handLabelY}, 20, 1,
             COLOR_TEXT_PRIMARY);

  if (interactionMode == INTERACTION_MODE_WIRING_SELECT_OUTPUT) {
    DrawTextEx(clientFont, "WIRING: Select Output",
               (Vector2){deckArea.x + UI_PADDING + handLabelTextWidth + 10,
                         handLabelY},
               20, 1, COLOR_ACCENT_PRIMARY);
  } else if (interactionMode == INTERACTION_MODE_WIRING_SELECT_INPUT) {
    DrawTextEx(clientFont,
               TextFormat("WIRING: From %d, Select Input", wiringFromElementId),
               (Vector2){deckArea.x + UI_PADDING + handLabelTextWidth + 10,
                         handLabelY},
               20, 1, COLOR_ACCENT_PRIMARY);
  }
  ...
  BeginScissorMode((int)deckArea.x, (int)cardAreaY, (int)deckArea.width,
                   (int)(deckArea.height - (cardAreaY - deckArea.y)));
  for (int i = 0; i < simulatorState->handCardCount; ++i) {
    Rectangle cardRect = {currentCardX, cardAreaY, CARD_WIDTH, CARD_HEIGHT};

    if (cardRect.x + cardRect.width > deckArea.x &&
        cardRect.x < deckArea.x + deckArea.width) {
      ...
      Rectangle cardTextRect = {cardRect.x + CARD_PADDING,
                                cardRect.y + CARD_PADDING,
                                cardRect.width - 2 * CARD_PADDING,
                                cardRect.height - 2 * CARD_PADDING};
      GuiLabel(cardTextRect, simulatorState->userHand[i].name);

      if (simulatorState->userHand[i].type == CARD_TYPE_ACTION) {
        Rectangle actionLabelRect = {cardRect.x + CARD_PADDING,
                                     cardRect.y + cardRect.height - 20,
                                     cardRect.width - 2 * CARD_PADDING, 15};
  ...
  if (simulatorState->handCardCount > MAX_VISIBLE_CARDS_IN_HAND) {
    float maxScroll =
        (simulatorState->handCardCount - MAX_VISIBLE_CARDS_IN_HAND) *
            (CARD_WIDTH + CARD_SPACING) +
        CARD_SPACING; // Added some padding
    DrawTextEx(
        clientFont, "<",
        (Vector2){deckArea.x + UI_PADDING, cardAreaY - 20 - UI_PADDING}, 20, 1,
        handScrollOffset > 0 ? COLOR_TEXT_PRIMARY : COLOR_TEXT_SECONDARY);
    DrawTextEx(clientFont, ">",
               (Vector2){deckArea.x + deckArea.width - UI_PADDING -
                             MeasureTextEx(clientFont, ">", 20, 1).x,
                         cardAreaY - 20 - UI_PADDING},
               20, 1,
               handScrollOffset < maxScroll ? COLOR_TEXT_PRIMARY
                                            : COLOR_TEXT_SECONDARY);
  }

  float scoreZoomTargetX = currentScreenWidth - 200;
  if (scoreZoomTargetX <
      playArea.x + playArea.width -
          200) { // Ensure it doesn't go too far left if play area is narrow
    scoreZoomTargetX = playArea.x + playArea.width - 200;
  }
  if (scoreZoomTargetX < UI_PADDING)
    scoreZoomTargetX = UI_PADDING;
  DrawTextEx(clientFont, TextFormat("Score: %d", simulatorState->score),
             (Vector2){scoreZoomTargetX, UI_HEADER_HEIGHT + UI_PADDING}, 20, 1,
             COLOR_TEXT_PRIMARY);
  DrawTextEx(
      clientFont, TextFormat("Zoom: %.2fx", gameCamera.zoom),
      (Vector2){scoreZoomTargetX, UI_HEADER_HEIGHT + UI_PADDING + 20 + 5}, 20,
      1, COLOR_TEXT_SECONDARY);
  DrawTextEx(
      clientFont,
      TextFormat("Target: (%.0f, %.0f)", gameCamera.target.x,
                 gameCamera.target.y),
      (Vector2){scoreZoomTargetX, UI_HEADER_HEIGHT + UI_PADDING + (20 + 5) * 2},
      20, 1, COLOR_TEXT_SECONDARY);
}
...
static void HandleGameplayInput(SimulatorState *simulatorState) {
  ...
  if (!gameplayHasLoggedEntry && simulatorState != NULL) {
    TraceLog(LOG_INFO,
             ...
             simulatorState->score, simulatorState->deckCardCount,
             simulatorState->currentDeckIndex, simulatorState->handCardCount,
  ...
  if (IsKeyPressed(KEY_D)) {
    if (Server_UserDrawCard(simulatorState)) {
      TraceLog(LOG_INFO,
               "CLIENT: User attempted to draw a card. Hand size now: %d",
               simulatorState->handCardCount);
    } else {
      TraceLog(LOG_INFO, "CLIENT: User tried to draw, but couldn't (hand full "
                         "or no cards left).");
    }
  }
  ...
  Rectangle playArea = {0, UI_HEADER_HEIGHT, (float)GetScreenWidth(),
                        (float)GetScreenHeight() - UI_HEADER_HEIGHT -
                            UI_DECK_AREA_HEIGHT};
  Rectangle deckArea = {0, (float)GetScreenHeight() - UI_DECK_AREA_HEIGHT,
  ...
  if (CheckCollisionPointRec(mousePosition, deckArea) &&
    ...
    if (wheel != 0) {
      ...
      if (handScrollOffset < 0)
        handScrollOffset = 0;
      if (handScrollOffset > maxScroll)
        handScrollOffset = maxScroll;
    }
  }
  ...
  if (GetTouchPointCount() > 0) {
    ...
    if (CheckCollisionPointRec(currentTouchPos, deckArea) &&
        simulatorState->handCardCount > MAX_VISIBLE_CARDS_IN_HAND) {
      if (!touchScrolling) {
  ...
  bool leftInputReleased = IsMouseButtonReleased(MOUSE_BUTTON_LEFT) ||
                           (GetTouchPointCount() == 0 && heldButtonId != -1 &&
  ...
  if (leftInputPressed) {
    ...
    Rectangle deckArea = {0, (float)GetScreenHeight() - UI_DECK_AREA_HEIGHT,
                          (float)GetScreenWidth(), UI_DECK_AREA_HEIGHT};
    if (CheckCollisionPointRec(inputPosition, deckArea) &&
        interactionMode == INTERACTION_MODE_NORMAL) {
      float currentCardX = deckArea.x + UI_PADDING;
      float cardAreaY = deckArea.y + UI_PADDING + 20 + UI_PADDING;
      for (int i = 0; i < simulatorState->handCardCount; ++i) {
        Rectangle cardRect = {currentCardX - handScrollOffset, cardAreaY,
        ...
        if (CheckCollisionPointRec(inputPosition, cardRect)) {
          ...
          if (selectedCardFromHand.type == CARD_TYPE_ACTION) {
            if (!turnInProgress) {
              TraceLog(LOG_INFO,
                       "CLIENT: Cannot play action cards outside of turn");
              ...
              actionsThisTurn++;
              TraceLog(LOG_INFO,
                       "CLIENT: Played action card '%s' (%d/%d actions)",
                       selectedCardFromHand.name, actionsThisTurn,
                       maxActionsPerTurn);
            }
          } else {
    ...
    else if (CheckCollisionPointRec(inputPosition, playArea)) {
      Vector2 worldInputPos = GetScreenToWorld2D(inputPosition, gameCamera);
      Vector2 gridPos = {floorf(worldInputPos.x / GRID_CELL_SIZE),
                         floorf(worldInputPos.y / GRID_CELL_SIZE)};
      if (interactionMode == INTERACTION_MODE_WIRING_SELECT_OUTPUT) {
        for (int i = 0; i < simulatorState->elementCount; ++i) {
          if (simulatorState->elementsOnCanvas[i].isActive &&
              (int)simulatorState->elementsOnCanvas[i].canvasPosition.x ==
                  (int)gridPos.x &&
              (int)simulatorState->elementsOnCanvas[i].canvasPosition.y ==
                  (int)gridPos.y) {
            wiringFromElementId = simulatorState->elementsOnCanvas[i].id;
            interactionMode = INTERACTION_MODE_WIRING_SELECT_INPUT;
            TraceLog(LOG_INFO,
                     "CLIENT: Wiring - Output selected from element ID %d",
                     wiringFromElementId);
            break;
          }
        }
        ...
        if (clickedElementId != -1 && clickedElementId != wiringFromElementId) {
          ...
          if (targetElement->type == ELEMENT_AND ||
              targetElement->type == ELEMENT_OR) {
          ...
          if (targetInputSlot != -1) {
            if (Server_CreateConnection(simulatorState, wiringFromElementId,
                                        clickedElementId, targetInputSlot)) {
              TraceLog(LOG_INFO, "CLIENT: Connection created");
            }
          } else {
            TraceLog(LOG_INFO,
                     "CLIENT: Target element has no available inputs");
          }
        }
        ...
        if (selectedCardIndex != -1) {
          if (simulatorState->userHand[selectedCardIndex].type ==
              CARD_TYPE_ELEMENT) {
            if (!turnInProgress) {
              TraceLog(LOG_INFO,
                       "CLIENT: Cannot place elements outside of turn");
              selectedCardIndex = -1;
            } else if (actionsThisTurn >= maxActionsPerTurn) {
              TraceLog(LOG_INFO, "CLIENT: Maximum actions per turn reached for "
                                 "placing element");
              selectedCardIndex = -1;
            } else {
              ...
              for (int k = 0; k < simulatorState->elementCount; ++k) {
                if (simulatorState->elementsOnCanvas[k].isActive &&
                    (int)simulatorState->elementsOnCanvas[k].canvasPosition.x ==
                        (int)gridPos.x &&
                    (int)simulatorState->elementsOnCanvas[k].canvasPosition.y ==
                        (int)gridPos.y) {
                  cellOccupied = true;
                  TraceLog(
                      LOG_WARNING,
                      "CLIENT: Canvas cell (%.0f, %.0f) is already occupied.",
                      gridPos.x, gridPos.y);
                  break;
                }
              }
              ...
              if (!cellOccupied &&
                  simulatorState->elementCount < MAX_ELEMENTS_ON_CANVAS) {
                Card cardToPlace = simulatorState->userHand[selectedCardIndex];
                CircuitElement *newElement =
                    &simulatorState
                         ->elementsOnCanvas[simulatorState->elementCount];
                ...
                TraceLog(LOG_INFO,
                         "CLIENT: Placed %s (ID: %d) at canvas (%.0f, %.0f)",
                         cardToPlace.name, newElement->id, gridPos.x,
                         gridPos.y);
                ...
                if (Server_UseCardFromHand(simulatorState, selectedCardIndex)) {
                  ...
                  TraceLog(LOG_INFO,
                           "CLIENT: Placed element '%s' (%d/%d actions)",
                ...
                TraceLog(LOG_WARNING,
                         "CLIENT: Max elements reached on canvas.");
                selectedCardIndex = -1;
              }
            }
          } else {
  ...
  if (heldButtonId != -1 &&
      (IsMouseButtonDown(MOUSE_BUTTON_LEFT) || GetTouchPointCount() > 0)) {
...
static Rectangle GetUIButtonBarRect(float screenWidth, float screenHeight) {
  float barHeight = 48;
  Rectangle deckArea = {0, screenHeight - UI_DECK_AREA_HEIGHT, screenWidth,
                        UI_DECK_AREA_HEIGHT};
  return (Rectangle){0, deckArea.y - barHeight, screenWidth, barHeight};
}

static bool DrawUIButton(Rectangle rect, const char *label, Color bg,
                         Color fg) {
  Vector2 inputPos =
      GetTouchPointCount() > 0 ? GetTouchPosition(0) : GetMousePosition();
  ...
  DrawRectangleLinesEx(rect, borderThickness,
                       isHovered ? COLOR_ACCENT_PRIMARY : DARKGRAY);

  // Draw button text with proper centering
  Vector2 textSize = MeasureTextEx(clientFont, label, 20, 1);
  Vector2 textPos = {rect.x + (rect.width - textSize.x) / 2,
  ...
  if (isHovered && inputPressed) {
    pressed = true;
    DrawRectangleRec(
        rect, Fade(COLOR_ACCENT_PRIMARY, 0.5f)); // Visual feedback for press
  }
...
static void DrawTouchUIAndHandle(SimulatorState *simulatorState) {
  ...
  if (DrawUIButton(turnBtn, turnInProgress ? "End Turn" : "Start Turn",
                   turnInProgress ? COLOR_ACCENT_SECONDARY : LIGHTGRAY,
                   COLOR_TEXT_PRIMARY)) {
    if (!turnInProgress) {
      turnInProgress = true;
      actionsThisTurn = 0;
...
void Client_UpdateAndDraw(SimulatorState *simulatorState) {
  if (currentClientScreen == CLIENT_SCREEN_LOADING) {
    ...
    if (IsKeyPressed(KEY_Q)) {
      currentClientScreen = CLIENT_SCREEN_SIMULATION;
      TraceLog(LOG_INFO, "CLIENT: Closing Scenario Details view, returning to "
                         "Simulation (Q key).");
    }
  ...
  if (currentClientScreen == CLIENT_SCREEN_LOADING) {
    ...
    if (simulatorState != NULL) {
      ...
      DrawTextEx(clientFont, "Error: SimulatorState is NULL for Details",
                 (Vector2){20, 20}, 20, 1, RED);
    }
  } else if (currentClientScreen == CLIENT_SCREEN_SIMULATION) {
    if (simulatorState != NULL) {
      ...
      DrawTextEx(clientFont, "Error: SimulatorState is NULL", (Vector2){20, 20},
                 20, 1, RED);
    }
  } else {
